...
4) Создать проект в SonarQube:
   - Нажать «Create a local project»
   - Project key: `lab3-project`
   - Display name: по варианту задания
   - Нажать «Set Up» → «Locally» → «Generate» для получения токена доступа

> ⚠️ **Важно:** сохраните токен сразу — он отображается только один раз.
...
```bash
# Через SonarScanner CLI:
sonar-scanner -Dsonar.token=ВАШ_ТОКЕН

# Или через Docker (без установки SonarScanner):
docker run --rm \
  --network host \
  -v $(pwd):/usr/src \
  sonarsource/sonar-scanner-cli \
  -Dsonar.token=ВАШ_ТОКЕН
```
